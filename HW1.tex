...
\hlkwd{canvas_collatz}\hldef{(}
  \hlkwc{colors} \hldef{=} \hlkwd{colorPalette}\hldef{(}\hlsng{"tuscany3"}\hldef{),}
...
\hlkwd{canvas_collatz}\hldef{(}
\hlkwc{colors} \hldef{=} \hlkwd{colorPalette}\hldef{(}\hlsng{"tuscany3"}\hldef{),}
\hlkwc{background} \hldef{=} \hlsng{"#dbdbdd"}\hldef{,}
\hlkwc{n} \hldef{=} \hlnum{122}\hldef{,}
\hlkwc{angle.even} \hldef{=} \hlnum{0.0223}\hldef{,}
\hlkwc{angle.odd} \hldef{=} \hlopt{-}\hlnum{0.25}\hldef{,}
\hlkwc{side} \hldef{=} \hlnum{FALSE}
\hldef{)}
\end{alltt}
\end{kframe}
...
\end{knitrout}
\caption{Fourth image created using collatz algorithm}
\label{CollatzPlot4}
\end{center}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{num.vec} \hlkwb{<-} \hlkwd{c}\hldef{(}\hlnum{0}\hlopt{:}\hlnum{10}\hldef{)}
\hldef{num.vec}
\end{alltt}
\begin{verbatim}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{alg.vec} \hlkwb{<-} \hlnum{2}\hlopt{^}\hldef{(num.vec} \hlopt{+} \hlnum{1}\hldef{)} \hlopt{+} \hlnum{2}\hlopt{^}\hldef{(num.vec} \hlopt{-} \hlnum{1}\hldef{)}
\hldef{alg.vec}
\end{alltt}
\begin{verbatim}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwd{which}\hldef{(alg.vec} \hlopt{==} \hlnum{40}\hldef{)}
\end{alltt}
\begin{verbatim}
## [1] 5
\end{verbatim}
...
\textbf{Solution:} This approach would not work for this problem because the solution is not an integer, and is this stored differently as data than the integers dealt with in the homework.
